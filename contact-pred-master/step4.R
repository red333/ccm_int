#!/usr/bin/Rscript
step4 <- function(prediction_table, truecontact_table, l1, l2, queryindex){
  ...
  predict_table <- read.table(prediction_table)
  truecontact <- read.table(truecontact_table)
  ...
  for(k in 1:4){
    ...
    summar <- paste(paste0("Number of TP: ", TP),
    paste0("Number of FP: ", FP),
    paste0("Precsion = ", P),
    paste0("Recall = ", R), sep = "\n")
    cat(summar, file = paste0(prediction_table, "_PR.txt"))
  }
}